...
class BaseModel():
    ...
    def __init__(self, config_json, training=True):
        ...
        self.regularizer = tf.keras.regularizers.L2(l2=self.weight_decay)
    ...
    def build_cnn(self):
        ...
        # 180x320
        self.conv1 = tf.compat.v1.layers.conv2d(inputs=self.input_images,
                                      filters=16,
                                      kernel_size=3,
                                      padding="same",
                                      ...
                                      name="conv1")
        self.pool1 = tf.compat.v1.layers.max_pooling2d(inputs=self.conv1,
                                             pool_size=2,
                                             strides=2,
                                             name="pool1")
        # 90x160
        self.conv2 = tf.compat.v1.layers.conv2d(inputs=self.pool1,
                                      filters=32,
                                      kernel_size=3,
                                      padding="same",
                                      activation=tf.nn.relu,
                                      ...
                                      bias_regularizer=self.regularizer,
                                      name="conv2")
        self.pool2 = tf.compat.v1.layers.max_pooling2d(inputs=self.conv2,
                                             pool_size=2,
                                             strides=2,
                                             name="pool2")
        # 45x80
        self.conv3 = tf.compat.v1.layers.conv2d(inputs=self.pool2,
                                      filters=64,
                                      kernel_size=3,
                                      padding="same",
                                      activation=tf.nn.relu,
                                      ...
                                      bias_regularizer=self.regularizer,
                                      name="conv3")
        self.pool3 = tf.compat.v1.layers.max_pooling2d(inputs=self.conv3,
                                             pool_size=2,
                                             strides=2,
                                             padding="same",
                                             name="pool3")
        # 23x40
        self.conv4 = tf.compat.v1.layers.conv2d(inputs=self.pool3,
                                      filters=64,
                                      kernel_size=3,
                                      padding="same",
                                      activation=tf.nn.relu,
                                      ...
                                      bias_regularizer=self.regularizer,
                                      name="conv4")
        self.pool4 = tf.compat.v1.layers.max_pooling2d(inputs=self.conv4,
                                             pool_size=2,
                                             strides=2,
                                             padding="same",
                                             name="pool4")
        # 12x20
        self.conv5 = tf.compat.v1.layers.conv2d(inputs=self.pool4,
                                      filters=64,
                                      kernel_size=3,
                                      padding="same",
                                      activation=tf.nn.relu,
                                      ...
                                      bias_regularizer=self.regularizer,
                                      name="conv5")
        self.pool5 = tf.compat.v1.layers.max_pooling2d(inputs=self.conv5,
                                             pool_size=2,
                                             strides=2,
                                             padding="same",
                                             name="pool5")
        ...
        self.pool3_heat = tf.compat.v1.layers.conv2d(inputs=self.pool3,
                                           filters=1,
                                           kernel_size=1,
                                           padding="same",
                                           activation=tf.nn.relu,
                                           ...
                                           bias_regularizer=self.regularizer,
                                           name="pool3_heat")
        self.pool4_heat = tf.compat.v1.layers.conv2d(inputs=self.pool4,
                                           filters=1,
                                           kernel_size=1,
                                           padding="same",
                                           activation=tf.nn.relu,
                                           ...
                                           bias_regularizer=self.regularizer,
                                           name="pool4_heat")
        self.pool5_heat = tf.compat.v1.layers.conv2d(inputs=self.pool5,
                                           filters=1,
                                           kernel_size=1,
                                           padding="same",
                                           activation=tf.nn.relu,
    ...
    def build_loss(self):
        # heatmap loss
        self.heatmap_loss = tf.compat.v1.losses.softmax_cross_entropy(
        ...
        #self.fc = tf.compat.v1.layers.Dense(self.interact_out_flat,
        #                          self.total_interacts,
        #                          activation=tf.nn.relu,
        #                          kernel_regularizer=self.regularizer,
        #                          bias_regularizer=self.regularizer,
        #                          name="fc")
        self.fc = tf.compat.v1.layers.dense(inputs=self.interact_out_flat,
                                            units=self.total_interacts,
                                            activation=tf.nn.relu,
                                            kernel_regularizer=self.regularizer,
                                            bias_regularizer=self.regularizer,
                                            name="fc")
        ...
        self.interact_loss = tf.compat.v1.losses.softmax_cross_entropy(self.true_interacts,
                                                             self.fc)
                                                             # self.fc_dropout)
...
class SingleScreenModel(BaseModel):
    ...
    def build_model(self):
        # do upsampling
        # 6x10
        self.pool5_up_filters = tf.compat.v1.get_variable("pool5_up_filters", [4, 4, 1, 1])
        self.pool5_up = tf.nn.relu(tf.compat.v1.nn.conv2d_transpose(value=self.pool5_heat,
        ...
        self.pool4_up_filters = tf.compat.v1.get_variable("pool4_up_filters", [4, 4, 1, 1])
        self.pool4_up = tf.nn.relu(tf.compat.v1.nn.conv2d_transpose(value=self.pool4_heat_sum,
        ...
        self.pool3_up_filters = tf.compat.v1.get_variable("pool3_up_filters", [16, 16, 1, 1])
        self.pool3_up = tf.nn.relu(tf.compat.v1.nn.conv2d_transpose(value=self.pool3_heat_sum,
...
class MultipleScreenModel(BaseModel):
    ...
    def build_model(self):
        ...
        # do upsampling
        # 6x10
        self.pool5_up_filters = tf.compat.v1.get_variable("pool5_up_filters", [4, 4, 1, 1], regularizer=self.regularizer)
        self.pool5_up = tf.nn.relu(tf.compat.v1.nn.conv2d_transpose(value=self.pool5_heat_out,
                                   filter=self.pool5_up_filters,
                                   output_shape=[self.batch_size, 12, 20, 1],
                                   strides=[1, 2, 2, 1],
                                   name="pool5_up"))
        ...
        self.pool4_up_filters = tf.compat.v1.get_variable("pool4_up_filters", [4, 4, 1, 1], regularizer=self.regularizer)
        self.pool4_up = tf.nn.relu(tf.compat.v1.nn.conv2d_transpose(value=self.pool4_heat_sum,
                                   filter=self.pool4_up_filters,
                                   output_shape=[self.batch_size, 23, 40, 1],
                                   strides=[1, 2, 2, 1],
        ...
        self.pool3_up_filters = tf.compat.v1.get_variable("pool3_up_filters", [16, 16, 1, 1], regularizer=self.regularizer)
        self.pool3_up = tf.nn.relu(tf.compat.v1.nn.conv2d_transpose(value=self.pool3_heat_sum,
                                   filter=self.pool3_up_filters,
                                   output_shape=[self.batch_size, 180, 320, 1],
                                   strides=[1, 8, 8, 1],
